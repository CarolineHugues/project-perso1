  ...
  function initMap() {
    $.getJSON('http://localhost/project-sass-music-travel/api/articles-data.php', function( data ) {
      ...
      for (var i = 0; i < data.result.articles.length; i++) {
        ...
        marker.bindPopup('<a href="http://localhost/project-sass-music-travel/article.php?id=/' + data.result.articles[i].id + '">' + data.result.articles[i].title + '</a>' + '<br/> ' + data.result.articles[i].description + '<br/> ' + '<img src="' + data.result.articles[i].image + '">');
